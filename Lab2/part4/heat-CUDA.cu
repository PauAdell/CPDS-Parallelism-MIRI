...
__global__ void gpu_Diff (float *h, float *g, float *diff, int N);
__global__ void gpu_Heat_reduction (float *h, float *g, int N);
...
int main( int argc, char *argv[] ) {
    ...
    float *dev_diffs, *dev_red1, *dev_red2;
    unsigned int imageSize = np*np*sizeof(float);
    
    // maximum number of threads per block is limited to 1024
    const int maxThreads = 1024;
    unsigned int numBlocks = Grid_Dim * Grid_Dim; 
    unsigned int numThreads = Block_Dim * Block_Dim; 
    if (REDUCTION && numBlocks > maxThreads){
        numThreads = maxThreads;
        numBlocks = ((np-2) * (np-2) / numThreads) + ((np-2) * (np-2) % numThreads != 0);
    }

    // TODO: Allocation on GPU for matrices u and uhelp
    // Device Allocation
    cudaMalloc(&dev_u, imageSize);
    cudaMalloc(&dev_uhelp, imageSize);
    if (REDUCTION) {
    	cudaMalloc(&dev_diffs, imageSize);
        cudaMalloc(&dev_red1, numThreads*sizeof(float));
        cudaMalloc(&dev_red2, numBlocks*sizeof(float));
    }

    // TODO: Copy initial values in u and uhelp from host to GPU
    //
    //          dest, host            from host to device
    cudaMemcpy(dev_u, param.u, imageSize, cudaMemcpyHostToDevice);
    cudaMemcpy(dev_uhelp, param.uhelp, imageSize, cudaMemcpyHostToDevice);
    
    float *res;
    if (REDUCTION){
    	res = (float*)malloc(sizeof(float)*numBlocks);
    }
    ...
    while(1) {
    	if (REDUCTION) {
            ...
            gpu_Heat_reduction<<<numBlocks, numThreads, numThreads*sizeof(float)>>>(dev_diffs, dev_red1, np * np);
            gpu_Heat_reduction<<<1, numBlocks, numBlocks*sizeof(float)>>>(dev_red1, dev_red2, numBlocks);
            cudaDeviceSynchronize();  // Wait for compute device to finish.
            cudaMemcpy(res, dev_red2, sizeof(float)*numBlocks, cudaMemcpyDeviceToHost);
            residual = res[0];
    	} else {
    	    // simple version
	    ...
	    //...
	    //         host, device           from device to host
	    cudaMemcpy(param.u, dev_u, imageSize, cudaMemcpyDeviceToHost);
	    cudaMemcpy(param.uhelp, dev_uhelp, imageSize, cudaMemcpyDeviceToHost);
